...
initializePerformanceOptimizations()}catch(error){console.warn("Portfolio initialization error:",error)}}
function handleLoading(){const loader=document.getElementById("loader");if(!loader){setupPageSpecificAnimations();return}
...
function staggerChildAnimations(parent){const children=parent.children;if(prefersReducedMotion){Array.from(children).forEach((child)=>child.classList.add("animate-in"));return}
...
return isValid}
function showFieldError(field,message){clearFieldError(field);field.classList.add("error");if(field.name==="message"){const warning=document.getElementById("message-warning");if(warning){warning.textContent=message;warning.style.display="block";return}}
...
function clearFieldError(field){field.classList.remove("error");if(field.name==="message"){const warning=document.getElementById("message-warning");if(warning){warning.textContent="";warning.style.display="none"}}
...
function setupBackToTop(){const backToTopButton=document.getElementById("back-to-top");if(!backToTopButton){console.warn("Back to top button not found");return}
...
if(isMobileDevice()){const webpSrc=transformedImage.replace(/\.(png|jpg|jpeg|avif)(\?v=\d+)?$/i,".webp?v=20251010");imgTag=`<picture>
...
`;list.innerHTML+=card})})})})();document.addEventListener("DOMContentLoaded",function(){const yearElement=document.querySelector('[itemprop="copyrightYear"]');if(yearElement){yearElement.textContent=new Date().getFullYear()}})
