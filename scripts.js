...
if ("requestIdleCallback" in window) {
    requestIdleCallback(addPreconnects);
} else {
    setTimeout(addPreconnects, 0);
}
...
const passiveIfSupported = supportsPassive() ? { passive: !0 } : !1;
    ...
    try {
        ...
        if (resumeModal) {
            try {
                ...
                console.warn("Resume modal setup failed:", error);
            }
        }
...
function handleLoading() {
    ...
    document.body.style.cssText =
        "overflow: hidden !important; position: fixed !important; top: 0 !important; left: 0 !important; width: 100% !important; height: 100vh !important; margin: 0 !important; padding: 0 !important;";
    document.documentElement.style.cssText = "overflow: hidden !important; height: 100% !important;";
    document.body.offsetHeight;
    document.addEventListener("touchmove", preventAllScrolling, { passive: !1 });
    document.addEventListener("wheel", preventAllScrolling, { passive: !1 });
    document.addEventListener("scroll", preventAllScrolling, { passive: !1 });
    document.addEventListener("keydown", preventScrollKeys, { passive: !1 });
...
function setupScrollProgressBar() {
    var oldBar = document.getElementById("scroll-progress");
...
function optimizeImages() {
    ...
    images.forEach((img) => {
        if (!img.hasAttribute("loading")) {
            img.loading = isMobile ? "lazy" : "lazy";
        }
        img.decoding = "async";
        img.style.opacity = "1";
        ...
        if (img.closest(".hero")) {
            ...
            img.loading = "eager";
            if (isMobile && img.src.includes("images/")) {
        ...
        img.onerror = function () {
            ...
            this.style.minHeight = isMobile ? "150px" : "200px";
            this.style.display = "block";
        };
        if (img.src) {
            img.dataset.originalSrc = img.src;
        }
        if (!img.closest(".hero") && img.dataset.src) {
    ...
    emailInput.addEventListener("input", function () {
        ...
        if (email) {
            if (tempMailDomains.includes(domain)) {
                ...
                !domain.match(
                    /^[a-z0-9.-]+\.(com|org|net|edu|gov|co|io|in|info|biz|us|ca|uk|au|de|fr|jp|cn|ru|br|za|eu)$/
                )
            ) {
...
function showNotification(message, type = "info") {
    ...
    const isIndexForm =
        ...
        window.location.pathname === "";
    const indexForm = document.getElementById("index-contact-form");
    if (isIndexForm && indexForm) {
        ...
        indexForm.style.position = "relative";
        indexForm.appendChild(notification);
    } else {
...
function optimizeFontLoading() {
    ...
    if (!isMobileDevice() || navigator.connection?.effectiveType === "4g") {
        const fontPreloads = [
...
if (supportsPassive()) {
    window.addEventListener("scroll", optimizedScrollHandler, { passive: !0 });
} else {
...
function setupResumeModal() {
    ...
    try {
        ...
        document.addEventListener("keydown", function (e) {
            if (e.key === "Escape" && resumeModal.classList && resumeModal.classList.contains("active")) {
                closeResumeModal();
            }
        });
    } catch (error) {
...
if (typeof module !== "undefined" && module.exports) {
    module.exports = { validateField, validateForm, showNotification, debounce, throttle };
}
const phrases = [
...
function initTypewriter() {
    ...
    function type() {
        ...
        if (!isDeleting) {
            el.textContent = phrase.slice(0, currentChar + 1);
            currentChar++;
        ...
        .then((projects) => {
            ...
            categories.forEach((category) => {
                ...
                grouped[category]
                    ...
                    .forEach((proj) => {
                        const techTags = proj.tech
                            .map((tag) => `<span class="tech-tag" itemprop="keywords">${tag}</span>`)
                            .join("\n");
                        ...
                        const dashboardBtn = hasDashboard
                            ? `<a href="${dashboardUrl}" target="_blank" class="btn btn-primary" itemprop="url">
                <i class="${dashboardIcon}"></i>${dashboardText}
                        ...
                        // Optimize Cloudinary images: inject w_400,q_auto for proper sizing
                        function getOptimizedCloudinaryUrl(url) {
                            if (typeof url !== 'string') return url;
                            // Only transform if it's a Cloudinary URL and not already transformed
                            if (url.includes('res.cloudinary.com') && url.includes('/upload/') && !/\/upload\/(w_\d+|q_auto)/.test(url)) {
                        ...
                        if (!firstProjectRendered && isMobileDevice()) {
                            const webpSrc = getOptimizedCloudinaryUrl(proj.image.replace(/\.png$/i, ".webp"));
                            const optimizedSrc = getOptimizedCloudinaryUrl(proj.image);
                            imgTag = `<picture>
  <source srcset="${webpSrc}" type="image/webp">
  <img src="${optimizedSrc}" alt="${proj.alt}" itemprop="image" />
</picture>`;
                            firstProjectRendered = !0;
                        } else {
                            const optimizedSrc = getOptimizedCloudinaryUrl(proj.image);
                            imgTag = `<img src="${optimizedSrc}" alt="${proj.alt}" loading="lazy" itemprop="image" />`;
                        }
                        const card = `
