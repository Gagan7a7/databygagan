...
const passiveIfSupported = supportsPassive() ? { passive: !0 } : !1; function supportsPassive() {
    let supportsPassive = !1; try { const opts = Object.defineProperty({}, "passive", { get: function () { supportsPassive = !0 }, }); window.addEventListener("testPassive", null, opts); window.removeEventListener("testPassive", null, opts) } catch (e) { }
    return supportsPassive
}
function initializePortfolio() {
    const fragment = document.createDocumentFragment(); try {
        handleLoading(); setupNavigation(); setupScrollEffects(); setupAnimations(); setupContactForm(); setupBackToTop(); setupThemeHandling(); const resumeModal = document.getElementById("resume-modal"); if (resumeModal) { try { setupResumeModal() } catch (error) { console.warn("Resume modal setup failed:", error) } }
        const typewriterElement = document.querySelector(".typewriter-text"); if (typewriterElement) { if (prefersReducedMotion) { typewriterElement.textContent = "Freelance Data Analyst" } else { const shouldRunTypewriter = optimizeForMobile(); if (shouldRunTypewriter !== !1) { requestAnimationFrame(() => { setTimeout(initTypewriter, isMobileDevice() ? 1000 : 500) }) } } }
        initializePerformanceOptimizations()
    } catch (error) { console.warn("Portfolio initialization error:", error) }
}
function handleLoading() {
    const loader = document.getElementById("loader"); if (!loader) { setupPageSpecificAnimations(); return }
    if (prefersReducedMotion) { loader.style.display = "none"; loader.classList.add("hidden"); document.body.style.cssText = ""; document.documentElement.style.cssText = ""; sessionStorage.setItem("portfolioLoaded", "true"); setupPageSpecificAnimations(); return }
    let hasLoaded = sessionStorage.getItem("portfolioLoaded"); if (hasLoaded) {
        if (loader) { loader.style.display = "none"; loader.classList.add("hidden") }
        document.body.style.cssText = ""; document.documentElement.style.cssText = ""; setupPageSpecificAnimations(); return
    }
    document.body.style.cssText = "overflow: hidden !important; position: fixed !important; top: 0 !important; left: 0 !important; width: 100% !important; height: 100vh !important; margin: 0 !important; padding: 0 !important;"; document.documentElement.style.cssText = "overflow: hidden !important; height: 100% !important;"; document.body.offsetHeight; document.addEventListener("touchmove", preventAllScrolling, { passive: !1 }); document.addEventListener("wheel", preventAllScrolling, { passive: !1 }); document.addEventListener("scroll", preventAllScrolling, { passive: !1 }); document.addEventListener("keydown", preventScrollKeys, { passive: !1 }); window.addEventListener("load", function () {
        setTimeout(function () {
            if (loader) { loader.classList.add("hidden"); sessionStorage.setItem("portfolioLoaded", "true") }
            document.body.style.cssText = ""; document.documentElement.style.cssText = ""; document.removeEventListener("touchmove", preventAllScrolling); document.removeEventListener("wheel", preventAllScrolling); document.removeEventListener("scroll", preventAllScrolling); document.removeEventListener("keydown", preventScrollKeys); setupPageSpecificAnimations()
        }, 1200)
    })
}
function preventAllScrolling(e) { const loader = document.getElementById("loader"); if (loader && !loader.classList.contains("hidden")) { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation(); return !1 } }
function preventScrollKeys(e) { const loader = document.getElementById("loader"); if (loader && !loader.classList.contains("hidden")) { const scrollKeys = [32, 33, 34, 35, 36, 37, 38, 39, 40]; if (scrollKeys.includes(e.keyCode)) { e.preventDefault(); e.stopPropagation(); return !1 } } }
...
function setupScrollEffects() {
    ...
    setupScrollProgressBar(); setupRevealAnimations()
}
...
function setupAnimations() { optimizeImages(); if (prefersReducedMotion) return; const observerOptions = { threshold: 0.1, rootMargin: "0px 0px -50px 0px" }; const observer = new IntersectionObserver(function (entries) { entries.forEach((entry) => { if (entry.isIntersecting) { entry.target.classList.add("animate-in"); if (entry.target.classList.contains("stagger-animation")) { staggerChildAnimations(entry.target) } } }) }, observerOptions); const animatedElements = document.querySelectorAll(".project-card, .service-card, .skill-category"); animatedElements.forEach((el) => { el.classList.add("animate-on-scroll"); observer.observe(el) }) }
function handleParallaxEffect() { if (prefersReducedMotion) return; const hero = document.querySelector(".hero"); const heroBackground = document.querySelector(".hero-background"); if (hero && heroBackground) { const scrolled = window.pageYOffset; const rate = scrolled * -0.5; heroBackground.style.transform = `translate3d(0, ${rate}px, 0)` } }
...
function setupScrollProgressBar() { var oldBar = document.getElementById("scroll-progress"); if (oldBar && oldBar.parentNode) { oldBar.parentNode.removeChild(oldBar) } }
function setupRevealAnimations() { if (prefersReducedMotion) return; const revealElements = document.querySelectorAll(".section-title, .about-text p, .contact-item, .skill-category h3, .service-card h3"); const revealObserver = new IntersectionObserver((entries) => { entries.forEach((entry) => { if (entry.isIntersecting) { entry.target.classList.add("reveal-animate"); revealObserver.unobserve(entry.target) } }) }, { threshold: 0.2, rootMargin: "0px 0px -50px 0px" }); revealElements.forEach((el) => { el.classList.add("reveal-element"); revealObserver.observe(el) }) }
function optimizeImages() {
    const images = document.querySelectorAll("img"); const isMobile = isMobileDevice(); const imageObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach((entry) => {
            if (entry.isIntersecting) {
                const img = entry.target; if (img.dataset.src) { img.src = img.dataset.src; img.removeAttribute("data-src") }
                img.style.opacity = "1"; observer.unobserve(img)
            }
        })
    }, { rootMargin: isMobile ? "50px 0px" : "100px 0px", threshold: 0.01 }); images.forEach((img) => {
        if (!img.hasAttribute("loading")) { img.loading = isMobile ? "lazy" : "lazy" }
...
function setupEmailDomainValidation() {
    const allowedDomains = ["gmail.com", "yahoo.com", "outlook.com", "hotmail.com", "protonmail.com", "icloud.com"]; const tempMailDomains = ["tempmail.com", "mailinator.com", "10minutemail.com", "guerrillamail.com", "getnada.com", "trashmail.com", "fakeinbox.com", "mintemail.com", "dispostable.com", "yopmail.com", "sharklasers.com", "spamgourmet.com", "maildrop.cc", "mytemp.email", "throwawaymail.com", "mailnesia.com",]; const emailInput = document.getElementById("email") || document.getElementById("index-email"); const warning = document.getElementById("email-warning") || document.getElementById("index-email-warning"); const form = document.getElementById("contact-form"); if (!emailInput || !warning || !form) return; emailInput.addEventListener("input", function () {
        const email = emailInput.value; const domain = email.substring(email.lastIndexOf("@") + 1).toLowerCase(); let errorMsg = ""; if (email) { if (tempMailDomains.includes(domain)) { errorMsg = "Temporary email addresses are not allowed." } else if (!allowedDomains.includes(domain) && domain !== "onmicrosoft.com" && !domain.match(/^[a-z0-9.-]+\.(com|org|net|edu|gov|co|io|in|info|biz|us|ca|uk|au|de|fr|jp|cn|ru|br|za|eu)$/)) { errorMsg = "Invalid email domain." } }
        if (errorMsg) { warning.textContent = errorMsg; warning.style.display = "block"; emailInput.setCustomValidity(errorMsg) } else { warning.style.display = "none"; emailInput.setCustomValidity("") }
    }); form.addEventListener("submit", function (e) {
        const email = emailInput.value; const domain = email.substring(email.lastIndexOf("@") + 1).toLowerCase(); let errorMsg = ""; if (email) { if (tempMailDomains.includes(domain)) { errorMsg = "Submission blocked: Temporary email addresses are not allowed." } else if (!allowedDomains.includes(domain) && domain !== "onmicrosoft.com" && !domain.match(/^[a-z0-9.-]+\.(com|org|net|edu|gov|co|io|in|info|biz|us|ca|uk|au|de|fr|jp|cn|ru|br|za|eu)$/)) { errorMsg = "Submission blocked: Invalid email domain." } }
        if (errorMsg) { e.preventDefault(); warning.textContent = errorMsg; warning.style.display = "block"; showNotification(errorMsg, "error"); return !1 }
    })
}
function handleFormSubmission(form, submitButton) {
    const originalText = submitButton.innerHTML; const formData = new FormData(form); if (!validateForm(form)) { showNotification("Please fill in all required fields correctly.", "error"); return }
    submitButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Sending...'; submitButton.disabled = !0; fetch(form.action, { method: "POST", body: formData, headers: { Accept: "application/json" } }).then((response) => { if (response.ok) { showNotification("Message sent successfully! I'll get back to you soon.", "success"); form.reset(); window.scrollTo({ top: 0, behavior: prefersReducedMotion ? "auto" : "smooth" }) } else { throw new Error("Network response was not ok") } }).catch((error) => { console.error("Error:", error); showNotification("There was a problem sending your message. Please try again later.", "error"); window.scrollTo({ top: 0, behavior: prefersReducedMotion ? "auto" : "smooth" }) }).finally(() => { submitButton.innerHTML = originalText; submitButton.disabled = !1 })
}
...
function validateField(field) {
    const value = field.value.trim(); const fieldType = field.type; let isValid = !0; let errorMessage = ""; if (field.hasAttribute("required") && !value) { isValid = !1; errorMessage = "This field is required." }
    ...
    if (field.name === "project-title" && value) { const titleRegex = /^[a-zA-Z0-9\s\-\.,:()]+$/; const spamPattern = /(.)\1{2,}|^(a{2,}|d{2,}|o{2,}|s{2,}|h{2,}|y{2,}|g{2,}|b{2,}|p{2,})$/i; if (!titleRegex.test(value) || spamPattern.test(value.replace(/\s+/g, ""))) { isValid = !1; errorMessage = "Project title looks invalid or spammy." } }
    if (field.name === "message" && value) { const wordCount = value.trim().split(/\s+/).length; if (wordCount < 2) { isValid = !1; errorMessage = "Please briefly describe your challenge (at least 2 words)." } }
    if (!isValid) { showFieldError(field, errorMessage) } else { clearFieldError(field) }
    return isValid
}
function showFieldError(field, message) {
    clearFieldError(field); field.classList.add("error"); if (field.name === "message") { const warning = document.getElementById("message-warning"); if (warning) { warning.textContent = message; warning.style.display = "block"; return } }
    const errorElement = document.createElement("span"); errorElement.className = "field-error"; errorElement.textContent = message; field.parentNode.appendChild(errorElement)
}
function clearFieldError(field) {
    field.classList.remove("error"); if (field.name === "message") { const warning = document.getElementById("message-warning"); if (warning) { warning.textContent = ""; warning.style.display = "none" } }
    const existingError = field.parentNode.querySelector(".field-error"); if (existingError && field.name !== "message") { existingError.remove() }
}
function showNotification(message, type = "info") {
    const notification = document.createElement("div"); notification.className = `notification notification-${type}`; const isIndexForm = window.location.pathname.includes("index.html") || window.location.pathname === "/" || window.location.pathname === ""; const indexForm = document.getElementById("index-contact-form"); if (isIndexForm && indexForm) { notification.style.position = "absolute"; notification.style.top = "auto"; notification.style.bottom = "100%"; notification.style.right = "20px"; notification.style.marginBottom = "10px"; notification.style.zIndex = "10000"; indexForm.style.position = "relative"; indexForm.appendChild(notification) } else { document.body.appendChild(notification) }
...
function setupBackToTop() {
    const backToTopButton = document.getElementById("back-to-top"); if (!backToTopButton) { console.warn("Back to top button not found"); return }
    backToTopButton.removeAttribute("style"); const throttledScrollHandler = throttle(function () { const scrolled = window.scrollY || document.documentElement.scrollTop; if (scrolled > 50) { backToTopButton.classList.add("visible") } else { backToTopButton.classList.remove("visible") } }, 16); window.addEventListener("scroll", throttledScrollHandler, passiveIfSupported); backToTopButton.addEventListener("click", function (e) { e.preventDefault(); window.scrollTo({ top: 0, behavior: prefersReducedMotion ? "auto" : "smooth" }) }); throttledScrollHandler()
}
...
function optimizeFontLoading() {
    const style = document.createElement("style"); const mobileOptimizations = isMobileDevice() ? `
...
function setupResumeModal() {
    const resumeModal = document.getElementById("resume-modal"); const closeResumeButton = document.getElementById("close-resume"); if (!resumeModal) { return }
...
function preventForcedReflows() {
    const elementCache = new Map(); const performanceMetrics = { scrollEvents: 0, cacheHits: 0, cacheMisses: 0 }; const originalGetBoundingClientRect = Element.prototype.getBoundingClientRect; Element.prototype.getBoundingClientRect = function () {
        const cacheKey = this.tagName + (this.id || "") + (this.className || ""); if (elementCache.has(cacheKey)) { performanceMetrics.cacheHits++; return elementCache.get(cacheKey) }
...
const phrases = ["Freelance Data Analyst", "n8n Automation Specialist", "Power BI Developer", "Excel Dashboard Specialist", "Data Cleaning Expert", "Python + AI Analyst",]; let currentPhrase = 0; let currentChar = 0; let isDeleting = !1; const typeSpeed = 100; const eraseSpeed = 50; const delayBetween = 1500; function initTypewriter() {
...
function setupPerformanceMonitoring() {
    ...
    const list = document.getElementById("projects-list"); const btns = document.getElementById("category-buttons"); if (!list || !btns) return; let apiUrl = "/api/projects"; if (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1") { apiUrl = "http://localhost:5000/api/projects" }
    fetch(apiUrl).then((res) => res.json()).then((projects) => {
        list.innerHTML = ""; btns.innerHTML = ""; const grouped = {}; projects.forEach((proj) => { const cat = proj.category || "Other"; if (!grouped[cat]) grouped[cat] = []; grouped[cat].push(proj) }); const categories = Object.keys(grouped); categories.forEach((category) => { const btn = document.createElement("button"); btn.textContent = category; btn.className = "category-scroll-btn"; btn.onclick = function () { const heading = document.getElementById("cat-" + category.replace(/\s+/g, "-").toLowerCase()); if (heading) { heading.scrollIntoView({ behavior: "smooth", block: "start" }) } }; btns.appendChild(btn) }); let firstProjectRendered = !1; categories.forEach((category) => {
            list.innerHTML += `<h2 class="project-category-heading" id="cat-${category.replace(/\s+/g, "-").toLowerCase()}">${category}</h2>`; grouped[category].slice().reverse().forEach((proj) => {
                ...
                if (isMobileDevice()) {
                    const webpSrc = transformedImage.replace(/\.(png|jpg|jpeg|avif)(\?v=\d+)?$/i, ".webp?v=20251010"); imgTag = `<picture>
  <source srcset="${webpSrc}" type="image/webp">
  <img src="${transformedImage}" alt="${proj.alt}" ${imgAttrs} itemprop="image" style="width: 100%; height: auto;" />
</picture>`} else { imgTag = `<img src="${transformedImage}" alt="${proj.alt}" ${imgAttrs} itemprop="image" style="width: 100%; height: auto;" />` }
                const card = `
